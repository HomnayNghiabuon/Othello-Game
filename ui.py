...
class Gui:
    def __init__(self):
        ...
        # display
        self.SCREEN_SIZE = (640, 550)  # Tăng chiều cao để chứa tỉ lệ thắng
        ...
        self.WIN_PROB_POS = (5, 460)  # Dưới bàn cờ với khoảng cách an toàn
        self.font = pygame.font.SysFont("Times New Roman", 22)
        self.scoreFont = pygame.font.SysFont("Serif", 58)
        self.probFont = pygame.font.SysFont("Arial", 16)
    ...
    def update(self, board, blacks, whites, current_player_color, black_win_prob=None, white_win_prob=None):
        """Updates screen
        """
        for i in range(8):
            for j in range(8):
        ...
        # Hiển thị tỉ lệ thắng nếu có
        if black_win_prob is not None and white_win_prob is not None:
            self.showWinProbability(black_win_prob, white_win_prob)
    ...
    def showWinProbability(self, black_win_prob, white_win_prob):
        """Hiển thị tỉ lệ thắng của cả hai bên"""
        # Xóa vùng hiển thị cũ
        clear_rect = pygame.Rect(0, self.WIN_PROB_POS[1], self.SCREEN_SIZE[0], 80)
        pygame.draw.rect(self.screen, self.BACKGROUND, clear_rect)

        # Hiển thị tỉ lệ thắng của BLACK
        black_prob_text = f"Black: {black_win_prob:.1%}"
        black_text = self.probFont.render(black_prob_text, True, self.WHITE)
        self.screen.blit(black_text, (self.BLACK_LAB_POS[0], self.WIN_PROB_POS[1]))

        # Hiển thị tỉ lệ thắng của WHITE
        white_prob_text = f"White: {white_win_prob:.1%}"
        white_text = self.probFont.render(white_prob_text, True, self.WHITE)
        self.screen.blit(white_text, (self.WHITE_LAB_POS[0], self.WIN_PROB_POS[1]))

        # Hiển thị thanh progress bar
        bar_width = 250
        bar_height = 15
        bar_x = (self.SCREEN_SIZE[0] - bar_width) // 2
        bar_y = self.WIN_PROB_POS[1] + 20
        ...
        # Vẽ phần BLACK (từ trái)
        black_width = int(bar_width * black_win_prob)
        if black_width > 0:
            pygame.draw.rect(self.screen, (50, 50, 50), (bar_x, bar_y, black_width, bar_height))

        # Vẽ phần WHITE (từ phải)
        white_width = int(bar_width * white_win_prob)
        if white_width > 0:
            white_start_x = bar_x + bar_width - white_width
            pygame.draw.rect(self.screen, (200, 200, 200), (white_start_x, bar_y, white_width, bar_height))

        # Thêm nhãn cho progress bar
        label_text = "Win Probability"
        label = self.probFont.render(label_text, True, self.WHITE)
        label_x = (self.SCREEN_SIZE[0] - label.get_width()) // 2
        self.screen.blit(label, (label_x, bar_y + 20))
